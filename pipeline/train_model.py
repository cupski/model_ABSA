...
def train_model(config: dict, data: dict) -> dict:
    ...
    dict dengan kunci:
      ...
      history     : riwayat loss dan F1 per epoch
...
def _train_indobert(config: dict, data: dict) -> dict:
    ...
    for epoch in range(params['num_epochs']):
        ...
        print(f"  Epoch {epoch+1}/{params['num_epochs']} | "
              ...
              f"Sent: {avg_sent:.4f}  <- metrik utama")
        ...
        if avg_sent > best_f1:
            ...
            print(f"    Best model tersimpan (Val Sentiment F1: {best_f1:.4f})")
        else:
            patience_cnt += 1
...
if __name__ == '__main__':
    ...
    SMOKE_CONFIG = {
        ...
        'model': {
            ...
            'params': {
                ...
                'batch_size'   : 4,
                'learning_rate': 2e-5,
                'num_epochs'   : 1,   # satu epoch untuk smoke test
                'warmup_ratio' : 0.1,
                'weight_decay' : 0.01,
                'max_grad_norm': 1.0,
                'patience'     : 1,
    ...
    # Gunakan hanya 20 sampel untuk smoke test
